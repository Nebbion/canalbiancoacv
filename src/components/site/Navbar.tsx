    ...
    <header className="sticky top-0 z-50 backdrop-blur-lg bg-secondary/90 border-b border-white/10">
      <nav className="max-w-7xl mx-auto px-4 md:px-8 h-16 md:h-20 flex items-center justify-between text-white">
        <Link to="/" className="flex items-center gap-3" onClick={() => setOpen(false)}>
          <img src={logo} alt="ASD Canalbianco ACV" width={40} height={40} className="h-9 w-9 md:h-11 md:w-11" />
          <div className="leading-tight">
            <div className="font-display text-lg md:text-xl tracking-wider">CANALBIANCO ACV</div>
            <div className="text-[10px] md:text-xs text-white/60 uppercase tracking-widest">A.S.D. dal 2013</div>
          </div>
        </Link>
          ...
          {links.map((l) => (
            <li key={l.to}>
              <Link
                ...
                activeProps={{ className: "px-3 py-2 text-sm font-medium uppercase tracking-wider text-white border-b-2 border-primary" }}
          ...
          <li>
            ...
            >
              <span className="relative flex h-2 w-2">
                <span className="absolute inset-0 rounded-full bg-white pulse-live" />
            ...
            {links.map((l) => (
              <li key={l.to}>
                <Link
                  ...
                  activeProps={{ className: "block px-3 py-3 rounded-md text-base font-medium uppercase tracking-wider bg-primary text-primary-foreground" }}
                  activeOptions={{ exact: l.to === "/" }}
                >
                  {l.label}
                </Link>
