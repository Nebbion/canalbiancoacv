...
function etichettaGirone(girone: string) {
  if (isSpeciale(girone)) return girone;
...
function TeamLogo({ nome, size = "md" }: { nome: string; size?: "sm" | "md" | "lg" }) {
...
function MatchCardSpeciale({ m, compact }: { m: Match; compact: boolean }) {
  ...
  return (
    <article className={`group relative rounded-xl text-white border ${borderColor} bg-gradient-to-br ${gradientBg} overflow-hidden hover:shadow-stadium transition`}>
      ...
      <div className="relative flex items-center gap-2.5 px-4 pt-4 pb-2">
        ...
        <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest ${badgeCls}`}>
          {m.stato === "In corso" && <span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-current" />}
          {m.stato}
        </span>
      </div>
      ...
      <div className="relative px-4 py-3">
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4">
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            <div className="font-display text-base md:text-lg tracking-wide leading-tight">{m.casa}</div>
          </div>
          ...
          <div className="min-w-[72px] text-center">
            {hasScore ? (
              ...
              <div className={`font-display text-xl font-light tracking-widest ${accentColor} opacity-60`}>VS</div>
            )}
          </div>

          <div className="flex flex-col items-center gap-2 text-center">
            <TeamLogo nome={m.ospite} size="lg" />
            <div className="font-display text-base md:text-lg tracking-wide leading-tight">{m.ospite}</div>
          </div>
        </div>
      </div>
      ...
      {!compact && m.marcatori.length > 0 && (
        <div className="relative border-t border-white/10 px-4 py-3 text-xs text-white/80">
          <div className={`font-bold uppercase tracking-widest text-[10px] mb-1 ${accentColor}`}>Marcatori</div>
          <ul className="space-y-0.5">
            {m.marcatori.map((g, i) => (
              <li key={i}>
                ⚽ {g.nome} {g.gol > 1 && `(×${g.gol})`}{" "}
