...
function isSpeciale(g: string) {
  return GIRONI_SPECIALI.includes((g ?? "").toLowerCase());
}
...
function TeamLogo({ nome, size = "sm" }: { nome: string; size?: "sm" | "md" }) {
  ...
  if (failed)
    return (
      <div className={`${dim} rounded-full bg-white/10 border border-white/15 flex items-center justify-center font-bold text-white/50 flex-shrink-0`}>
        {iniziali(nome)}
      </div>
    );
  return (
    ...
    <div className={`rounded-xl border overflow-hidden ${isFinale ? "border-amber-400/40" : "border-white/10"} bg-white/5`}>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-t border-white/8 text-[11px] text-white/40">
        ...
        <span className={`uppercase tracking-widest font-bold ${
          m.stato === "In corso" ? "text-primary" :
          m.stato === "Terminata" ? "text-white/40" : "text-white/30"
        }`}>{m.stato}</span>
      </div>
    </div>
  );
...
function TabelloneCategoria({ categoria, matches }: { categoria: string; matches: Match[] }) {
  const semifinali = matches.filter((m) => m.girone.toLowerCase() === "semifinale");
  const finali     = matches.filter((m) => m.girone.toLowerCase() === "finale");
  const isFinale   = finali.length > 0;

  return (
    <div className={`rounded-xl border overflow-hidden ${isFinale ? "border-amber-400/30" : "border-blue-400/20"}`}>
      {/* Header categoria */}
      <div className={`px-5 py-3 flex items-center gap-3 border-b border-white/10 ${isFinale ? "bg-amber-500/10" : "bg-blue-500/8"}`}>
        <span className="text-lg">{isFinale ? "🏆" : "⚔️"}</span>
        <div>
          <div className="text-[10px] uppercase tracking-widest text-white/50">Fase finale</div>
          <div className="font-display text-xl text-white">{categoria}</div>
        </div>
        <span className={`ml-auto text-[10px] font-bold uppercase tracking-[2px] px-2.5 py-1 rounded-full border ${
          isFinale ? "bg-amber-500/15 text-amber-300 border-amber-400/30" : "bg-blue-500/15 text-blue-300 border-blue-400/30"
        }`}>
      ...
      <div className="p-4 space-y-6">
        ...
        {semifinali.length > 0 && finali.length > 0 && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-white/30 text-xs uppercase tracking-widest">Finale</span>
            <div className="h-px flex-1 bg-white/10" />
          </div>
        )}
        ...
        {finali.length > 0 && (
          <div className="max-w-sm mx-auto">
            {finali.map((m) => <TabelloneMatch key={m.id} m={m} />)}
          </div>
        )}
      </div>
    </div>
...
function Classifiche() {
  ...
  const gironiNormali = Object.entries(data?.standings ?? {})
    .filter(([key]) => !isSpeciale(key.split("|")[1] ?? ""))
    .filter(([, rows]) => rows.some((r) => r.pg > 0));

  // Partite di fase finale raggruppate per categoria
  const matchesFinale = (data?.matches ?? []).filter((m) => isSpeciale(m.girone));
      ...
      {gironiNormali.length > 0 && (
        <div className="grid gap-8 lg:grid-cols-2">
          {gironiNormali.map(([key, rows]) => {
            ...
            return <ClassificaGirone key={key} categoria={categoria} girone={girone} rows={rows} />;
          })}
        </div>
      )}
