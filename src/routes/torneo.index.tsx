...
function TorneoLive() {
  ...
  if (!liveConfigured) {
    return (
      <SectionWrap>
        <section className="rounded-xl border border-white/10 bg-white/5 p-8 md:p-10">
          <div className="inline-flex items-center gap-2 text-primary text-sm font-bold uppercase tracking-widest">
            <Archive size={16} /> Torneo live
          </div>
          <h2 className="mt-3 font-display text-3xl md:text-5xl tracking-wide">
            Nuova edizione in preparazione
          </h2>
          <p className="mt-4 max-w-2xl text-white/70">
            Calendario, risultati, classifiche e marcatori live saranno
            riattivati quando sarà configurato il nuovo foglio del torneo.
          </p>
          <Link
            to="/archivio/tornei/trevisan-26"
            className="mt-6 inline-flex items-center rounded-md bg-primary px-5 py-3 text-sm font-bold uppercase tracking-wider text-primary-foreground shadow-glow hover:brightness-110"
          >
            Vai all'archivio 2026
          </Link>
        </section>
      </SectionWrap>
    );
      ...
      <div className="grid gap-6 md:grid-cols-[1.5fr_1fr] mb-12">
        <div className="rounded-xl bg-white/5 border border-white/10 p-6">
          ...
          <div className="mt-4 grid grid-cols-3 gap-4">
            {[
              { k: "Categorie", v: "3" },
              {
                ...
                v: String(
                  new Set([
                    ...(data?.matches ?? []).flatMap((m) => [m.casa, m.ospite]),
                  ]).size,
                ),
              },
              ...
              <div
                key={s.k}
                className="rounded-lg bg-secondary border border-white/10 p-3 text-center"
              >
          ...
          <ul className="mt-6 grid sm:grid-cols-3 gap-2 text-sm">
            {[
              "Pulcini · ore 19:00",
              "Esordienti · ore 20:00",
              "Giovanissimi · ore 21:00",
            ].map((x) => (
              <li
                key={x}
                className="rounded-md bg-secondary border-l-2 border-primary px-3 py-2"
              >
                {x}
              </li>
            ))}
        ...
        <div className="rounded-xl bg-white/5 border border-white/10 p-6 flex items-center gap-4">
          ...
          <div>
            <div className="inline-flex items-center gap-2 text-primary text-sm font-bold uppercase tracking-widest">
              <QrCode size={14} /> Condividi
            </div>
            <h3 className="font-display text-xl tracking-wide mt-1">
              Inquadra per i live
            </h3>
            <p className="text-xs text-white/70 mt-1">
              Apri la pagina torneo da qualsiasi smartphone.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <section className="mb-12">
      <div className="flex items-end justify-between mb-4">
        <h2
          className={`font-display text-2xl md:text-3xl tracking-wide ${accent ? "text-primary text-glow" : ""}`}
        >
          {title}
        </h2>
        {action}
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="h-44 rounded-xl bg-white/5 border border-white/10 animate-pulse"
        />
      ))}
    </div>
  );
